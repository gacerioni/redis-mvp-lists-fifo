...
batch_size = 250000  # Batch size for each LPUSH operation
...
def inject_messages(list_size):
    ...
    for i in range(num_lists):
        redis_client.delete(f"{source_queue_base}_{i}")
    ...
    for list_index in range(num_lists):
        list_name = f"{{{source_queue_base}_{list_index}}}"  # Correctly formatted with hash tags
